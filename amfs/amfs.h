...
#define BAD_FLAG "user.bad"
#define VERSION_NUMBER "user.version"
...
/*mount patternDB + lower path*/
struct amfs_opt_data {
	void *device_name;
	char *pattern_file;
};
...
static inline void amfs_set_lower_super(struct super_block *sb,
					  struct super_block *val, char *pattern_db,struct list_head *listhead,long value)
{
	AMFS_SB(sb)->lower_sb = val;
	AMFS_SB(sb)->pattern_db = pattern_db;
	AMFS_SB(sb)->first = listhead;
	AMFS_SB(sb)->version = value;
}
